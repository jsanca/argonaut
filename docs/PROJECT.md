...
Argonaut is a comparative laboratory for agentic AI frameworks. It runs the same mission through Spring AI, LangChain4j, LangGraph4j, Embabel, and Koog — keeping the external model, controlled evidence, conceptual tools, observable API contract, and evaluation conditions as consistent as practical — to produce honest, evidence-based comparison of how each framework naturally implements the same intelligent behavior.

## Scope

### In scope

- Five framework implementations executing the same agentic mission: Spring AI, LangChain4j, LangGraph4j, Embabel, Koog.
...
- A Hybrid implementation until evidence from the five current implementations exists.
...
Maven multi-module project targeting Java 25. Each framework runs as an independent service on its assigned port. See [`../README.md`](../README.md) for the module layout and port assignments.
...
- **Frameworks under comparison:** Spring AI, LangChain4j, LangGraph4j, Embabel, Koog
- **Frontend:** Vue.js
- **Model provider:** OpenRouter
- **Development environment:** Docker Compose with nginx reverse proxy
- **Build/test commands:** `mvn verify`, `npm test`, `npm run build`
...
Open `http://localhost:8080` once the stack is healthy. The UI accesses backends through nginx's same-origin `/api/frameworks/<id>/` paths.
